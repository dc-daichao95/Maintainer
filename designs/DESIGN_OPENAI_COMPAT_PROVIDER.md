...
"openai" | "openai-compatible" => {
    ...
    let max_tokens = settings.ai.openai_compat
        ...
        .unwrap_or(4096);
    let streaming = settings.ai.openai_compat
        .as_ref()
        .map(|c| c.streaming)
        .unwrap_or(false);
    let stream_idle_timeout_secs = settings.ai.openai_compat
        .as_ref()
        .map(|c| c.stream_idle_timeout_secs)
        .unwrap_or(DEFAULT_OPENAI_STREAM_IDLE_TIMEOUT_SECS);
...
OpenAI-compatible providers can enable internal SSE streaming without changing the public `AiProvider::generate_content()` contract. The client accumulates `content`, `tool_calls`, and `usage` chunks, then returns a complete `AiResponse` to the existing worker flow.
...
`stream_idle_timeout_secs` guards against stalled streams between chunks. Set it to `0` to disable the guard. If a gateway does not support `stream_options.include_usage`, the client retries the streaming request once without `stream_options`.
