...
struct OpenAiClientConfig {
    base_url: String,
    provider_type: openai::OpenAiProviderType,
    model: String,
...
fn openai_client_config(settings: &Settings) -> Result<OpenAiClientConfig> {
    let provider_type = match settings.ai.provider.to_lowercase().as_str() {
        "openai" => openai::OpenAiProviderType::OpenAi,
        "openai-compatible" => openai::OpenAiProviderType::OpenAiCompatible,
        p => bail!("Unsupported OpenAI provider: {}", p),
    };

    let openai_settings = settings.ai.openai_compat.as_ref();
    let base_url = openai_settings
        .and_then(|c| c.base_url.clone())
        .unwrap_or_else(|| {
            openai::OpenAiCompatClient::default_base_url_for_model(&settings.ai.model)
        });
    let context_window_size = openai_settings
        .and_then(|c| c.context_window_size)
        .unwrap_or_else(|| {
            openai::OpenAiCompatClient::default_context_window_for_model(&settings.ai.model)
        });
    let max_tokens = openai_settings.and_then(|c| c.max_tokens).unwrap_or(4096);
    let streaming = openai_settings.map(|c| c.streaming).unwrap_or(false);
    let stream_idle_timeout_secs = openai_settings
        .map(|c| c.stream_idle_timeout_secs)
        .unwrap_or(crate::settings::DEFAULT_OPENAI_STREAM_IDLE_TIMEOUT_SECS);
    ...
    Ok(OpenAiClientConfig {
        ...
        context_window_size,
        max_tokens,
        api_timeout_secs: settings.ai.api_timeout_secs,
        streaming,
...
pub fn create_provider(settings: &Settings) -> Result<Arc<dyn AiProvider>> {
    match settings.ai.provider.to_lowercase().as_str() {
        ...
        "openai" | "openai-compatible" => {
            let config = openai_client_config(settings)?;
...
mod tests {
    ...
    #[test]
    fn test_openai_client_config_reads_streaming_settings() -> Result<()> {
        let mut settings = Settings::new().expect("Failed to load settings");
        settings.ai.provider = "openai-compatible".to_string();
        settings.ai.model = "glm-4".to_string();
        settings.ai.openai_compat = Some(crate::settings::OpenAiCompatSettings {
        ...
        assert_eq!(config.context_window_size, 64_000);
        assert_eq!(config.max_tokens, 8192);
        assert!(config.streaming);
        assert_eq!(config.stream_idle_timeout_secs, 42);

        Ok(())
    }

    #[test]
    fn test_openai_client_config_uses_streaming_defaults() -> Result<()> {
        let mut settings = Settings::new().expect("Failed to load settings");
        settings.ai.provider = "openai".to_string();
        settings.ai.model = "gpt-4o".to_string();
        settings.ai.openai_compat = None;

        let config = openai_client_config(&settings)?;

        assert_eq!(config.provider_type, openai::OpenAiProviderType::OpenAi);
        assert!(!config.streaming);
        assert_eq!(
            config.stream_idle_timeout_secs,
            crate::settings::DEFAULT_OPENAI_STREAM_IDLE_TIMEOUT_SECS
        );
