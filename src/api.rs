...
pub struct AppState {
    ...
    stats_findings_cache: AsyncCache<serde_json::Value>,
...
pub struct Pagination {
    ...
    /// Comma-separated `key:direction` sort tokens (e.g. `pending:desc,date:asc`).
    /// Only whitelisted keys are honored; invalid tokens are ignored.
...
#[derive(Deserialize)]
pub struct FindingTriageRequest {
    pub id: i64,
    pub status: String,
...
#[derive(Deserialize)]
pub struct FindingCommentDeleteRequest {
    pub id: i64,
}

#[derive(Serialize)]
pub struct FindingTriageResponse {
    pub status: String,
}
...
pub async fn run_server(
    ...
    let state = Arc::new(AppState {
        ...
        stats_findings_cache: AsyncCache::new(Duration::from_secs(60)),
    ...
    let app = Router::new()
        ...
        .route("/api/stats/findings", get(stats_findings))
        .route("/api/submit", post(submit_patch))
        .route("/api/finding/triage", post(update_finding_triage))
        .route("/api/finding/comment/delete", post(delete_finding_comment))
...
async fn update_finding_triage(
    State(state): State<Arc<AppState>>,
    Json(payload): Json<FindingTriageRequest>,
) -> Result<Json<FindingTriageResponse>, StatusCode> {
    if state.read_only {
        return Err(StatusCode::FORBIDDEN);
    }

    let status = payload.status.trim();
    if !matches!(status, "pending" | "not_issue" | "to_fix" | "fixed") {
        return Err(StatusCode::BAD_REQUEST);
    }
    ...
    let updated = state
        .db
        .update_finding_triage(payload.id, status, comment)
        .await
        .map_err(|e| {
            error!("Failed to update finding triage: {}", e);
    ...
    state.patchsets_count_cache.invalidate().await;
    state.stats_timeline_cache.invalidate_all().await;
    state.stats_findings_cache.invalidate().await;

    Ok(Json(FindingTriageResponse {
        status: "updated".to_string(),
    }))
}

async fn delete_finding_comment(
    State(state): State<Arc<AppState>>,
    Json(payload): Json<FindingCommentDeleteRequest>,
) -> Result<Json<FindingTriageResponse>, StatusCode> {
    if state.read_only {
        return Err(StatusCode::FORBIDDEN);
    }
    ...
    let deleted = state
        .db
        .delete_finding_triage_comment(payload.id)
        .await
        .map_err(|e| {
            error!("Failed to delete finding triage comment: {}", e);
            StatusCode::INTERNAL_SERVER_ERROR
        })?;
...
async fn list_patchsets(
    ...
    let has_sort = pagination.sort.is_some();

    let mut items = if pagination.q.is_none()
    ...
    enrich_patchset_branches(&state, &mut items).await;
...
async fn enrich_patchset_branches(state: &AppState, items: &mut [crate::db::PatchsetRow]) {
    for item in items {
        let branch_is_missing_or_sha = item
            .baseline_branch
            .as_deref()
            .map(|branch| is_hex_sha(branch))
            .unwrap_or(true);

        if !branch_is_missing_or_sha {
            continue;
        }

        let Some(commit) = item
            .baseline_commit
            .as_deref()
            .or(item.baseline_branch.as_deref())
        else {
            continue;
        };
        ...
        match crate::git_ops::find_branches_containing_commit(&state.repo_path, commit).await {
            Ok(branches) if !branches.is_empty() => {
                item.baseline_branch = Some(branches.join(", "));
            }
            Ok(_) => {}
            Err(e) => {
                tracing::warn!("Failed to resolve branches for {}: {}", commit, e);
            }
        }
    }
}
...
async fn stats_findings(
    State(state): State<Arc<AppState>>,
) -> Result<Json<serde_json::Value>, StatusCode> {
    let data = state
        .stats_findings_cache
        .get_or_fetch(|| async {
            state.db.get_finding_triage_stats().await.map_err(|e| {
                info!("Error getting finding triage stats: {}", e);
                StatusCode::INTERNAL_SERVER_ERROR
            })
        })
        .await?;
