...
pub struct PatchsetRow {
    ...
    pub findings_pending: Option<i64>,
    pub findings_to_fix: Option<i64>,
...
impl Database {
    ...
    pub async fn migrate(&self) -> Result<()> {
        ...
        let _ = self
            .try_add_column("findings", "triage_status", "TEXT DEFAULT 'pending'")
            .await;
        let _ = self
            .try_add_column("findings", "triage_comment", "TEXT")
            .await;
        let _ = self
            .try_add_column("findings", "triage_updated_at", "INTEGER")
            .await;
        let _ = self
            .conn
            .execute(
                "UPDATE findings
                 ...
                 WHERE triage_status IS NULL
                    OR (triage_status = 'to_fix' AND triage_updated_at IS NULL)",
                (),
            )
            .await;
        let _ = self
            .conn
            .execute(
                "CREATE TABLE IF NOT EXISTS finding_triage_comments (
        ...
        let _ = self
            .try_create_index(
                "idx_finding_triage_comments_finding_id",
                "finding_triage_comments",
                "finding_id",
            )
            .await;
        let _ = self
            .conn
            .execute(
                "INSERT INTO finding_triage_comments (finding_id, status, comment, created_at)
                 SELECT f.id,
                        COALESCE(f.triage_status, 'pending'),
                        f.triage_comment,
                        COALESCE(f.triage_updated_at, 0)
                 FROM findings f
                 WHERE f.triage_comment IS NOT NULL
                   AND TRIM(f.triage_comment) != ''
                   AND NOT EXISTS (
                       SELECT 1 FROM finding_triage_comments c
                       WHERE c.finding_id = f.id
                         AND c.comment = f.triage_comment
                         AND c.created_at = COALESCE(f.triage_updated_at, 0)
                   )",
                (),
            )
            .await;
        ...
        let _ = self
            .try_add_column("reviews", "started_at", "INTEGER")
            .await;
        let _ = self
                ...
                .execute(
                    "UPDATE reviews
                     SET status = ?,
                         ...
                         END
                     WHERE id = ?",
                    libsql::params![status, l, status, now, review_id],
                )
                .await?;
        } else {
            self.conn
                .execute(
                    "UPDATE reviews
                     SET status = ?,
                         ...
                         END
                     WHERE id = ?",
                    libsql::params![status, status, now, review_id],
                )
                .await?;
        }
        Ok(())
    ...
    pub async fn create_finding(&self, finding: Finding) -> Result<()> {
        ...
        self.conn
            .execute(
                "INSERT INTO findings (review_id, severity, severity_explanation, problem, preexisting, locations, server_id, source_server)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                libsql::params![
                    finding.review_id,
    ...
    pub async fn get_timeline_stats(&self, subsystem_id: Option<i64>) -> Result<serde_json::Value> {
        ...
        if let Some(sid) = subsystem_id {
            ...
            JOIN patchsets_subsystems ps ON r.patchset_id = ps.patchset_id
            WHERE ps.subsystem_id = ?
              AND COALESCE(f.triage_status, 'pending') != 'not_issue'
            ...
            FROM findings f
            JOIN reviews r ON f.review_id = r.id
            WHERE COALESCE(f.triage_status, 'pending') != 'not_issue'
            GROUP BY day, severity
            ORDER BY day";
            let mut rows = self.conn.query(sql, ()).await?;
            while let Ok(Some(row)) = rows.next().await {
    ...
    pub async fn get_finding_triage_stats(&self) -> Result<serde_json::Value> {
        let sql = "SELECT
            CASE
                WHEN triage_status IN ('pending', 'not_issue', 'to_fix', 'fixed') THEN triage_status
                ELSE 'pending'
            END as status,
            COUNT(*) as count
        FROM findings
        WHERE COALESCE(preexisting, 0) = 0
        GROUP BY status";
        ...
        while let Ok(Some(row)) = rows.next().await {
            let status: String = row.get(0).unwrap_or_else(|_| "pending".to_string());
            let count: i64 = row.get(1)?;
            match status.as_str() {
                "pending" => pending = count,
                "not_issue" => not_issue = count,
                "fixed" => fixed = count,
                "to_fix" => to_fix = count,
                _ => pending += count,
            }
        ...
        Ok(json!({
            "total": pending + not_issue + to_fix + fixed,
            "pending": pending,
            "not_issue": not_issue,
            "to_fix": to_fix,
            "fixed": fixed
    ...
    pub async fn get_baseline(&self, id: i64) -> Result<Option<BaselineRow>> {
        let mut rows = self
            .conn
            .query(
        ...
        if let Ok(Some(row)) = rows.next().await {
            Ok(Some(BaselineRow {
                repo_url: row.get(0).ok(),
                branch: row.get(1).ok(),
                last_known_commit: row.get(2).ok(),
            }))
        } else {
            Ok(None)
        }
    ...
    pub async fn get_patchsets(
        ...
        self.get_patchsets_sorted(limit, offset, query, mailing_list, None)
            .await
    }

    /// Maps user-facing sort keys to safe SQL expressions. Returns a complete
    /// `ORDER BY ...` clause. Input is parsed from a comma-separated list of
    /// `key:direction` tokens (e.g. `pending:desc,date:asc`). Only whitelisted
    /// keys and directions are honored, so the result is always injection-safe.
    fn build_order_by(sort: Option<&str>) -> String {
        const ALLOWED: &[(&str, &str)] = &[
            ...
            ("date", "p.date"),
            ("status", "p.status"),
            ("pending", "COALESCE(f.pending, 0)"),
            ("to_fix", "COALESCE(f.to_fix, 0)"),
        ];
        ...
        if let Some(sort) = sort {
            for token in sort.split(',') {
                ...
                let dir = parts.next().unwrap_or("").trim().to_ascii_lowercase();
                if let Some((_, expr)) = ALLOWED.iter().find(|(k, _)| *k == key) {
                    let dir_sql = if dir == "asc" { "ASC" } else { "DESC" };
        ...
        if clauses.is_empty() {
            "ORDER BY p.date DESC".to_string()
        } else {
            // Stable tie-breaker so pagination stays deterministic.
            clauses.push("p.id DESC".to_string());
            format!("ORDER BY {}", clauses.join(", "))
        }
    }

    pub async fn get_patchsets_sorted(
        &self,
        limit: usize,
        offset: usize,
        query: Option<String>,
        ...
        let order_by = Self::build_order_by(sort.as_deref());
        ...
        let sql = format!(
            "SELECT p.id, p.subject, p.status, p.thread_id, p.author, p.date, p.cover_letter_message_id, p.total_parts, p.received_parts, GROUP_CONCAT(s.name, ','),
             COALESCE(f.low, 0), COALESCE(f.medium, 0), COALESCE(f.high, 0), COALESCE(f.critical, 0), COALESCE(f.pending, 0), COALESCE(f.to_fix, 0), p.baseline_id, p.failed_reason, p.target_review_count, p.skip_filters, p.only_filters,
             p.embargo_until, b.branch, b.last_known_commit
             FROM patchsets p
             ...
             LEFT JOIN (
                SELECT r.patchset_id,
                    ...
                    SUM(CASE WHEN COALESCE(f.triage_status, 'pending') = 'pending' AND COALESCE(f.preexisting, 0) = 0 THEN 1 ELSE 0 END) as pending,
                    SUM(CASE WHEN COALESCE(f.triage_status, 'pending') = 'to_fix' AND COALESCE(f.preexisting, 0) = 0 THEN 1 ELSE 0 END) as to_fix
                FROM reviews r
        ...
        loop {
            match rows.next().await {
                Ok(Some(row)) => {
                    ...
                    let embargo_until: Option<i64> = row.get(21).ok();
                    ...
                    let (low, medium, high, critical, pending, to_fix) = if is_embargoed {
                        (0, 0, 0, 0, 0, 0)
                    } else {
                        (
                            ...
                            row.get::<Option<i64>>(14).ok().flatten().unwrap_or(0),
                            row.get::<Option<i64>>(15).ok().flatten().unwrap_or(0),
                        )
                    };
                    ...
                    patchsets.push(PatchsetRow {
                        ...
                        findings_pending: Some(pending),
                        findings_to_fix: Some(to_fix),
                        baseline_id: row.get(16).ok(),
                        baseline_branch: row.get(22).ok(),
                        baseline_commit: row.get(23).ok(),
                        failed_reason: row.get(17).ok(),
                        target_review_count: row.get(18).ok(),
                        skip_filters: row.get(19).ok(),
                        only_filters: row.get(20).ok(),
                        model_name: None,
                        prompts_git_hash: None,
                        baseline_logs: None,
                        provider: None,
                        embargo_until: row.get(21).ok(),
                    });
                }
                Ok(None) => break,
                Err(e) => {
        ...
        if let Ok(Some(row)) = rows.next().await {
            ...
            let query_str = format!(
                "SELECT r.summary, r.created_at, ai.input_context, ai.output_raw, 
                        ...
                        r.started_at, r.completed_at, ai.created_at
                 FROM reviews r
            ...
            while let Ok(Some(r)) = rev_rows.next().await {
                let review_id = r.get::<i64>(11).ok();
                let findings = if let Some(id) = review_id {
                    self.get_findings_for_review(id).await.unwrap_or_default()
                } else {
                    Vec::new()
                };
                reviews.push(serde_json::json!({
                    ...
                    "started_at": r.get::<Option<i64>>(13).ok(),
                    "completed_at": r.get::<Option<i64>>(14).ok(),
                    "interaction_created_at": r.get::<Option<i64>>(15).ok(),
        ...
        if let Ok(Some(row)) = rows.next().await {
            ...
            while let Ok(Some(r)) = rev_rows.next().await {
                ...
                reviews.push(serde_json::json!({
                    ...
                    "started_at": r.get::<Option<i64>>(11).ok(),
                    "completed_at": r.get::<Option<i64>>(12).ok(),
                    "interaction_created_at": r.get::<Option<i64>>(13).ok(),
                    "model": model_name.clone(),
                    "provider": provider.clone(),
                    "prompts_hash": prompts_git_hash.clone(),
                    "baseline": baseline.clone(),
    ...
    async fn get_findings_for_review(&self, review_id: i64) -> Result<Vec<serde_json::Value>> {
        let mut rows = self
            .conn
            .query(
                "SELECT id, severity, severity_explanation, problem, suggestion,
                        COALESCE(triage_status, 'pending'), triage_comment, triage_updated_at
                 FROM findings
                 WHERE review_id = ?
        ...
        while let Ok(Some(row)) = rows.next().await {
            let severity_num: i64 = row.get(1).unwrap_or(1);
            ...
            let finding_id: i64 = row.get(0)?;
            let triage_status: String = row.get(5).unwrap_or_else(|_| "pending".to_string());
            let triage_comment: Option<String> = row.get(6).ok();
            let triage_updated_at: Option<i64> = row.get(7).ok();
            let triage_comments = self
                .get_finding_triage_comments(
                    finding_id,
                    &triage_status,
                    triage_comment.as_deref(),
                    triage_updated_at,
                )
                .await
                .unwrap_or_default();
            findings.push(serde_json::json!({
                ...
                "suggestion": row.get::<Option<String>>(4).ok(),
                "triage_status": triage_status,
                "triage_comment": triage_comment,
                "triage_updated_at": triage_updated_at,
    ...
    async fn get_finding_triage_comments(
        &self,
        finding_id: i64,
        legacy_status: &str,
        legacy_comment: Option<&str>,
        legacy_updated_at: Option<i64>,
    ) -> Result<Vec<serde_json::Value>> {
        let mut rows = self
            .conn
            .query(
                "SELECT id, status, comment, created_at
                 FROM finding_triage_comments
                 WHERE finding_id = ?
                 ORDER BY created_at ASC, id ASC",
                libsql::params![finding_id],
            )
            .await?;
        ...
        while let Ok(Some(row)) = rows.next().await {
            comments.push(serde_json::json!({
                "id": row.get::<i64>(0).ok(),
                "status": row.get::<String>(1).unwrap_or_else(|_| "pending".to_string()),
                "comment": row.get::<String>(2).unwrap_or_default(),
                "created_at": row.get::<i64>(3).ok(),
            }));
        }

        if comments.is_empty()
            && let Some(comment) = legacy_comment.map(str::trim).filter(|s| !s.is_empty())
        {
            comments.push(serde_json::json!({
    ...
    pub async fn update_finding_triage(
        ...
        let changed = self
            ...
            .execute(
                "UPDATE findings
                 SET triage_status = ?,
                     triage_comment = COALESCE(?, triage_comment),
                     triage_updated_at = ?
                 WHERE id = ?",
                libsql::params![status, comment, now, finding_id],
            )
            .await?;
        if changed > 0
            && let Some(comment) = comment.map(str::trim).filter(|s| !s.is_empty())
        {
            self.conn
                .execute(
                    "INSERT INTO finding_triage_comments (finding_id, status, comment, created_at)
                     VALUES (?, ?, ?, ?)",
                    libsql::params![finding_id, status, comment, now],
                )
                .await?;
        }
        Ok(changed > 0)
    ...
    pub async fn delete_finding_triage_comment(&self, comment_id: i64) -> Result<bool> {
        let mut rows = self
            .conn
            .query(
                "SELECT finding_id FROM finding_triage_comments WHERE id = ?",
                libsql::params![comment_id],
            )
            .await?;
        let finding_id = if let Some(row) = rows.next().await? {
            row.get::<i64>(0)?
        } else {
            return Ok(false);
        };
        ...
        let changed = self
            .conn
            .execute(
                "DELETE FROM finding_triage_comments WHERE id = ?",
                libsql::params![comment_id],
            )
            .await?;
        ...
        if changed > 0 {
            let mut rows = self
                .conn
                .query(
                    "SELECT COUNT(*) FROM finding_triage_comments WHERE finding_id = ?",
                    libsql::params![finding_id],
                )
                .await?;
            let remaining = if let Some(row) = rows.next().await? {
    ...
    pub async fn get_review_details(&self, id: i64) -> Result<Option<serde_json::Value>> {
        ...
        if let Ok(Some(r)) = rows.next().await {
            ...
            let findings = self.get_findings_for_review(review_id).await?;
            Ok(Some(serde_json::json!({
                "id": review_id,
                ...
                "started_at": r.get::<Option<i64>>(19).ok(),
                "completed_at": r.get::<Option<i64>>(20).ok(),
                "interaction_created_at": r.get::<Option<i64>>(21).ok(),
                "findings": findings,
            })))
        } else {
            Ok(None)
    ...
    pub async fn get_pending_patchsets(&self, limit: usize) -> Result<Vec<PatchsetRow>> {
        let mut rows = self.conn.query(
            "SELECT id, subject, status, thread_id, author, date, cover_letter_message_id, total_parts, received_parts, baseline_id, failed_reason, target_review_count, skip_filters, only_filters, embargo_until
             FROM patchsets WHERE status = 'Pending' ORDER BY date DESC LIMIT ?",
        ...
        while let Ok(Some(row)) = rows.next().await {
            patchsets.push(PatchsetRow {
                ...
                findings_pending: None,
                findings_to_fix: None,
