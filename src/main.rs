...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let log_tz: Option<chrono_tz::Tz> = settings_result
        .as_ref()
        .ok()
        .and_then(|s| s.review.schedule.as_ref())
        .and_then(|sched| sched.timezone.as_ref())
    ...
    if plain_logs {
        builder
            ...
            .without_time()
            .init();
    } else if let Some(tz) = log_tz {
        builder
            .with_timer(sashiko::logging::TzTimer::new(tz))
            .init();
    } else {
        builder.init();
    }
    ...
    let api_repo_path = std::path::PathBuf::from(&settings.git.repository_path);
...
async fn process_parsed_article(
    ...
    let baseline_id = if let Some(b) = baseline {
        match worker_db
            .create_baseline(repo_url.as_deref(), None, Some(&b))
            .await
        {
            Ok(id) => Some(id),
            Err(e) => {
    ...
    let (thread_id, is_git_import, git_import_total) =
        if let Some(rest) = group.strip_prefix("git-import:") {
            ...
            // Even for singletons, we must use article_id@sashiko.local as root to match the fetching placeholder!
            let root_msg_id = format!("{}@sashiko.local", article_id);
