...
fn default_openai_stream_idle_timeout_secs() -> u64 {
    DEFAULT_OPENAI_STREAM_IDLE_TIMEOUT_SECS
}
...
pub struct ScheduleSettings {
    ...
    pub end_hour: u8,
    /// IANA timezone name (e.g. "Asia/Shanghai"). Falls back to system local time if unset.
    #[serde(default)]
...
impl ScheduleSettings {
    pub fn validate(&self) -> Result<(), String> {
        if self.start_hour > 23 {
            return Err(format!(
                "start_hour {} is out of range, must be 0-23",
                self.start_hour
            ));
        }
        if self.end_hour > 23 {
            return Err(format!(
                "end_hour {} is out of range, must be 0-23",
                self.end_hour
            ));
        }
        if let Some(tz) = &self.timezone {
            if tz.parse::<chrono_tz::Tz>().is_err() {
                return Err(format!(
                    "invalid timezone '{}', use IANA names like 'Asia/Shanghai' or 'US/Pacific'",
                    tz
                ));
            }
        }
...
mod tests {
    ...
    #[test]
    fn openai_streaming_settings_default_to_disabled() {
    ...
    #[test]
    fn openai_streaming_settings_can_be_enabled() {
        let settings: OpenAiCompatSettings = toml::from_str(
            r#"
            streaming = true
        ...
        assert!(settings.streaming);
        assert_eq!(settings.stream_idle_timeout_secs, 42);
    }
}
