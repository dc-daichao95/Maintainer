...
Follow the formatting rules strictly. Do not use markdown headers or ALL CAPS shouting. Ensure the tone is constructive and professional. Do not use backticks to quote any names or expressions.\n\nLANGUAGE REQUIREMENT: Write the report comments in Simplified Chinese by default. Keep established technical terms, API names, function names, symbols, and commit metadata in their original form (usually English)."
            }
            _ => "",
        };
...
impl Worker {
    ...
    fn effective_input_limit(&self) -> usize {
        let caps = self.provider.get_capabilities();
        let reserve = if caps.context_window_size >= 20_000 {
            (caps.context_window_size / 20).max(1024)
        } else {
            caps.context_window_size / 20
        }
        .max(1)
        .min(caps.context_window_size.saturating_sub(1));
        let provider_limit = caps.context_window_size.saturating_sub(reserve);

        if self.max_input_tokens == 0 {
            provider_limit
        } else {
            self.max_input_tokens.min(provider_limit)
        }
    }

    fn truncate_request_to_input_budget(&self, request: &mut AiRequest, label: &str) -> Result<()> {
        let limit = self.effective_input_limit();
        let initial_tokens = self.provider.estimate_tokens(request);
        if initial_tokens <= limit {
            return Ok(());
        }

        for _ in 0..8 {
            let current_tokens = self.provider.estimate_tokens(request);
            if current_tokens <= limit {
                warn!(
                    "{} input truncated from {} to {} tokens (limit {})",
                    label, initial_tokens, current_tokens, limit
                );
                return Ok(());
            }

            let mut largest: Option<(Option<usize>, usize)> = request
                .system
                .as_ref()
                .map(|content| (None, TokenBudget::estimate_tokens(content)));
            ...
            for (idx, msg) in request.messages.iter().enumerate() {
                if let Some(content) = msg.content.as_ref() {
                    ...
                    if largest
                        .as_ref()
                        .is_none_or(|(_, largest_tokens)| tokens > *largest_tokens)
                    {
                        largest = Some((Some(idx), tokens));
                    }
                }
            }

            let Some((target_idx, largest_tokens)) = largest else {
                break;
            };
            if largest_tokens == 0 {
                break;
            ...
            let overhead_tokens = current_tokens.saturating_sub(largest_tokens);
            let allowed_tokens = limit.saturating_sub(overhead_tokens).max(1);
            let target_content = match target_idx {
                Some(idx) => request.messages[idx].content.as_mut(),
                None => request.system.as_mut(),
            };

            if let Some(content) = target_content {
                let truncated = Truncator::truncate_diff(content, allowed_tokens);
        ...
        let final_tokens = self.provider.estimate_tokens(request);
        if final_tokens > limit {
            return Err(ReviewError::BudgetExceeded(format!(
                "{} input is {} tokens after truncation (limit: {}, configured max_input_tokens: {})",
                label, final_tokens, limit, self.max_input_tokens
            ))
            .into());
        }
        ...
        warn!(
            "{} input truncated from {} to {} tokens (limit {})",
            label, initial_tokens, final_tokens, limit
        );
        Ok(())
    }
...
```"#,
    ...
    async fn run_ai_stage_raw(
        ...
        loop {
            ...
            let mut request = crate::ai::AiRequest {
            ...
            self.truncate_request_to_input_budget(&mut request, &format!("stage {}", _stage))?;
    ...
    async fn json_request(
        ...
        let mut req = req;
        self.truncate_request_to_input_budget(&mut req, label)
            .map_err(|e| {
                warn!("{} input budget exceeded: {}", label, e);
                e
            })
            .ok()?;
        ...
        match try_parse(content, &validate) {
            ...
            Err(e) => {
                ...
                if let Err(e) = self.truncate_request_to_input_budget(&mut retry_req, label) {
                    warn!("{} retry input budget exceeded: {}", label, e);
                    return None;
                }
...
mod tests {
    ...
    struct MockCountingProvider {
        context_window_size: usize,
    }

    #[async_trait::async_trait]
    impl crate::ai::AiProvider for MockCountingProvider {
        async fn generate_content(
            &self,
            _request: crate::ai::AiRequest,
        ) -> anyhow::Result<crate::ai::AiResponse> {
            anyhow::bail!("mock: should not generate")
        }

        fn estimate_tokens(&self, request: &crate::ai::AiRequest) -> usize {
            let mut total = 0;
            if let Some(system) = &request.system {
                total += crate::ai::token_budget::TokenBudget::estimate_tokens(system);
            }
            for msg in &request.messages {
                if let Some(content) = &msg.content {
                    total += crate::ai::token_budget::TokenBudget::estimate_tokens(content);
                }
            }
            total
        }
        ...
        fn get_capabilities(&self) -> crate::ai::ProviderCapabilities {
            crate::ai::ProviderCapabilities {
                model_name: "mock-counting".to_string(),
                context_window_size: self.context_window_size,
            }
        }
    }

    fn test_worker(max_input_tokens: usize, context_window_size: usize) -> Worker {
        let temp_dir = tempfile::tempdir().unwrap();
        let prompts_dir = temp_dir.path().join("prompts");
        std::fs::create_dir_all(&prompts_dir).unwrap();
        let provider = std::sync::Arc::new(MockCountingProvider {
            context_window_size,
        });
    ...
    #[test]
    fn test_effective_input_limit_uses_config_when_smaller_than_context() {
        let worker = test_worker(180_000, 200_000);
        assert_eq!(worker.effective_input_limit(), 180_000);
    }

    #[test]
    fn test_effective_input_limit_reserves_context_window_headroom() {
        let worker = test_worker(900_000, 200_000);
        assert_eq!(worker.effective_input_limit(), 190_000);
    }

    #[test]
    fn test_truncate_request_to_input_budget_reduces_large_message() {
        let worker = test_worker(100, 1_000);
        let mut request = crate::ai::AiRequest {
            system: None,
            messages: vec![AiMessage {
                role: AiRole::User,
                content: Some("large input ".repeat(1_000)),
                thought: None,
                thought_signature: None,
                tool_calls: None,
                tool_call_id: None,
        ...
        worker
            .truncate_request_to_input_budget(&mut request, "test")
            .unwrap();

        assert!(worker.provider.estimate_tokens(&request) <= worker.effective_input_limit());
        assert!(
            request.messages[0]
                .content
                .as_ref()
